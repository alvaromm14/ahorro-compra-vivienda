<script>
  ...
  const yScale = d3.scaleLinear()
    .domain([0, d3.max(data, d => d.Esfuerzo)])
    .range([innerHeight, 0]);

  $: lineAsequibilidad = d3.line()
  .curve(d3.curveBasis)
    .x((d) => xScale(d.Fecha))
    .y((d) => yScale(d.Esfuerzo));
...
<h1>Años de ahorro necesarios del hogar mediano para pagar una vivienda media</h1>
<div class="chart-container">
  <svg {width} {height}>
    <g class="inner-chart" transform="translate({margin.left}, {margin.top})">
      {#each [0,3,6,9] as tick, i}
       {#if i !== 0}
          <g class="tick" transform="translate(0, {yScale(tick)})">
              <text text-anchor="start" dominant-baseline="bottom" x="-40" y="-10" dy="0.35em">{tick}{i === 3 ? " años" : ""}</text>
